...
int  ransac::fittingPlane(double p, double w, double threshold)
{
    ...
    int numPar = 3;//Æ½ÃæÄ£ÐÍµÄ²ÎÊýÓÐ3¸ö
    int index[3];
...
//       std::cout<<"pt1 = "<<(ptcld->operator[](index[0])) <<std::endl
//                <<"pt2 = "<<(ptcld->operator[](index[1]))<<std::endl
//                <<"pt3 = "<<(ptcld->operator[](index[2]))<<std::endl<<std::endl
      ...
      totalDepth = 0;
       for(size_t i = 0; i < ptcld->getSize(); i++)
       {
		  ...
		  */
          ...
          double t2 = ((*ptcld)[i])[1];
          double t3 = ((*ptcld)[i])[2];
          dis_up = Plane.at<double>(0,0)*t1 + Plane.at<double>(1,0)*t2 + Plane.at<double>(2,0) - t3;
...
//	   std::cout<<"inner points = "<<current_best_points<<std::endl;
 ...
 void ransac::printPara()
 {
   if(this->model != nullptr)
     {
       std::cout<<"plane model is: a="<<model[0]<<", b="<<model[1]<<" , c="<<model[2]<<std::endl;
     }
 }
